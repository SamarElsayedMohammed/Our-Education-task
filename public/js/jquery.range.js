...
(function ($, window, document, undefined) {
    ...
    jRange.prototype = {
        ...
        isSingle: function () {
            ...
            return this.options.value.indexOf(",") !== -1 ||
                this.options.isRange
                ? false
                : true;
        },
        ...
        barClicked: function (e) {
            ...
            if (this.isSingle())
                ...
                var firstLeft = Math.abs(
                    ...
                    lastLeft = Math.abs(
                        parseFloat(this.pointers.last().css("left"), 10)
                    ),
        ...
        correctPositionForSnap: function (position) {
            var currentValue =
                this.positionToValue(position) - this.options.from;
        ...
        setValue: function (value) {
            ...
            values[0] =
                Math.min(
                    Math.max(values[0], this.options.from),
                    this.options.to
                ) + "";
            if (values.length > 1) {
                values[1] =
                    Math.min(
                        Math.max(values[1], this.options.from),
                        this.options.to
                    ) + "";
            }
            this.options.value = value;
            var prc = this.valuesToPrc(
                values.length === 2 ? values : [0, values[0]]
            );
            if (this.isSingle()) {
        ...
        valuesToPrc: function (values) {
            var lowPrc =
                    ((parseFloat(values[0]) - parseFloat(this.options.from)) *
                        100) /
                    this.interval,
                highPrc =
                    ((parseFloat(values[1]) - parseFloat(this.options.from)) *
                        100) /
                    this.interval;
        ...
        positionToValue: function (pos) {
            ...
            if (this.isDecimal()) {
                var final =
                    Math.round(
                        Math.round(value / this.options.step) *
                            this.options.step *
                            100
                    ) / 100;
                if (final !== 0.0) {
